...
def plot_loss_curves(train_losses, val_losses, out_path, title="Training and validation loss", val_accuracies=None):
    ...
    if val_accuracies is not None and len(val_accuracies) == len(train_losses):
        ax2 = ax.twinx()
        ax2.plot(epochs, np.asarray(val_accuracies) * 100.0, label="Val accuracy (%)", color="tab:green", linestyle="--", marker="^", linewidth=1.2)
...
def plot_confusion_matrix(cm, out_path, title="Confusion Matrix", class_names=None, normalize=False):
    out_path = Path(out_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    disp = ConfusionMatrixDisplay(confusion_matrix=matrix, display_labels=list(class_names))
    disp.plot(ax=ax, xticks_rotation=45, colorbar=True, values_format=values_format)
    ax.set_title(title)
    fig.tight_layout()
    fig.savefig(out_path, dpi=200)
    plt.close(fig)
...
def plot_augmentation_examples(originals, view1s, view2s, save_path, n_rows=10):
    save_augmentation_grid(originals, view1s, view2s, out_path=save_path, max_rows=n_rows)


def plot_similarity_heatmap(matrix, out_path, title="Cosine similarity", batch_size=None):
    ...
    if batch_size is not None:
        ...
        if matrix.shape[0] == 2 * n:
            ax.add_patch(Rectangle((n - 0.5, -0.5), n, n, fill=False, edgecolor="black", linewidth=0.8, linestyle="--"))
            ax.add_patch(Rectangle((-0.5, n - 0.5), n, n, fill=False, edgecolor="black", linewidth=0.8, linestyle="--"))
    fig.tight_layout()
    fig.savefig(out_path, dpi=200)
    plt.close(fig)
...
def plot_accuracy_curves(curves, out_path, title="Validation accuracy"):
    out_path = Path(out_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    fig, ax = plt.subplots(figsize=(8, 5))
...
def save_2d_feature_plot(
    ...
    """Save a t-SNE visualization of features.

    Labels are used only for coloring the plot, not for SSL training.
    """
    out_path = Path(out_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
